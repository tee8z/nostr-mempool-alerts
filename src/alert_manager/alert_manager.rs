...
async fn process_block_data(
    ...
    let alerts_to_update = active_alerts
        .iter()
        .map(|alert| handle_alert(alert.clone(), new_block.clone()))
...
fn send_alert_to_nostr(alert: &Alert, nostr_com: Channels<String>) -> Result<(), anyhow::Error> {
    let alert_string = alert.to_string().map_err(|e| anyhow::Error::new(e))?;
...
fn handle_alert(alert: Alert, new_block: MempoolData) -> Option<Alert> {
    let alert_updated = match alert.kind {
        AlertKind::ConfirmHeight => alert.kind.update_confirm_height_alert(alert.clone(), new_block),
        AlertKind::FeeLevel => alert.kind.update_fee_level_alert(alert.clone(), new_block),
        AlertKind::BlockHeight => alert.kind.update_block_height_alert(alert.clone(), new_block),
    };
